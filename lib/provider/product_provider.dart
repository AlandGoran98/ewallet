...
import 'package:ewallet/provider/app_brain.dart';
...
class Products with ChangeNotifier {
  ...
  Future<void> fetchAndSetProducts([bool filterByUser = false]) async {
    ...
    // try {
    Uri url = Uri.parse(
      'https://ewallet-1de65-default-rtdb.firebaseio.com/products.json?auth=$authToken',
    ); //to create a new products.json if its not there as a file to save
    http.Response response = await http.get(url);
    Map<String, dynamic> exractedData = jsonDecode(response.body);
    ...
    // url = Uri.parse(
    //     'https://ewallet-1de65-default-rtdb.firebaseio.com/userFavorites/$userId.json?auth=$authToken');
  ...
  Future<void> updateProduct(String id, AppData newProduct) async {
    ...
    if (prodIndex >= 0) {
      try {
        final Uri url = Uri.parse(
          'https://ewallet-1de65-default-rtdb.firebaseio.com/products/$id.json?auth=$authToken',
        ); //to create a new pr
